...
# %% prepare dataset -- slicing at 0.2
...
time_treat, mg_treat = np.tanh(time_steps.flatten()-1), np.tanh(mg_data.flatten()-1)

step_split = int(len(mg_treat)/2)
step_trans = 1000
slicing = int(0.2/h)
end_step = step_split + 2000

training_data, testing_data = mg_treat[:step_split], mg_treat[step_split:end_step]

# %% prepare alternative dataset -- slicing at 0.5
...
mg_ax.plot(mg_treat, lw=0.7)
mg_ax.axvline(len(training_data), c='black')
...
ld_range = [10**(-4), 10**(-6), 10**(-10), 10**(-14)]
gamma_range = np.arange(1, 10, 0.1)
spec_rad_range = np.arange(1, 10, 0.1)

for ld_val in ld_range:
    for gamma_val in gamma_range:
        for spec_val in spec_rad_range:
            ld = ld_val
            gamma = gamma_val
            spec_rad = spec_val
            s = 0
            ...
            state_dict = esn_help.listening(training_data, x_0, A, gamma, C, s, zeta, d, N)
            reg_result = esn_help.regression_covariance(ld, state_dict, step_trans)
            predicting = esn_help.prediction(state_dict, reg_result, testing_data, A, gamma, C, s, zeta, d, N)
            
            print(ld, gamma, spec_rad, predicting['testing_error'])
...
C = esn_help.gen_matrix(shape=(N,d), density=1, sd=2, mean=-1, pdf="uniform", seeded=False)
A = esn_help.gen_matrix(shape=(N,N), density=0.001, sd=2, mean=-1, pdf="uniform", seeded=False)
A = esn_help.spectral_radius_matrix(A, spec_rad)
...
esn_help.hist_accuracy_plot(z_actuals, z_predictions, 'f_name.pdf')

# %% compute lyapunov spectrum for ESM

x_data = predicting['states'].transpose()[0:2000, :]
...
%time mg_spectrum = lyaps_spec.lyapunov_spectrum(mg_data[0:2000, :], disc, disc_step, "difference", steps_trans_mg, jacobian_mg_disc)

# %% compare lyapunov spectrums for mackey glass and esn

plt.figure(figsize=(10, 5))

mg_spectrum_sorted = np.sort(mg_spectrum)[::-1][0:50]
esn_spectrum_sorted = np.sort(mg_esn_spectrum)[::-1][0:50]
