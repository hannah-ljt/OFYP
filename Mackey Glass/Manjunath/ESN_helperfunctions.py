...
def observation_mj(x_t, x_t_1, reg_result):
    w = reg_result[0]
    bias = reg_result[1]
    x_curr = np.concatenate((x_t, x_t_1), axis=0)
    z_next = np.transpose(w) @ x_curr + bias
    return z_next    # outputs (d, 1) array
...
def regression_mj(ld, state_dict, T_trans):
    ...
    X_2x = np.concatenate((X[:, 1:T], X[:, 0:T-1]), axis=0)[:, T_trans:]
    Z_2x = Z[:, 1:T][:, T_trans:]
    
    w_best = np.linalg.solve(X_2x @ X_2x.transpose() + ld * np.identity(2*N), X_2x @ Z_2x.transpose())
    a_best = (np.mean(Z_2x, axis=1) - (w_best.transpose() @ np.mean(X_2x, axis=1).reshape(2*N, 1))).reshape(d, 1)
...
def regression_covariance_mj(ld, state_dict, T_trans):
    X = state_dict['all_states']
    Z = state_dict['input_data']
    ...
    cov_XZ_2x = (1/T) * (X_2x @ Z_2x.transpose()) - (np.mean(X_2x, axis=1).reshape(2*N, 1) @ np.mean(Z_2x, axis=1).reshape(d, 1).transpose())
    cov_XX_2x = (1/T) * (X_2x @ X_2x.transpose()) - (np.mean(X_2x, axis=1).reshape(2*N, 1) @ np.mean(X_2x, axis=1).reshape(2*N, 1).transpose()) + ld * np.identity(2*N)
    
    w_best = np.linalg.solve(cov_XX_2x, cov_XZ_2x)
    a_best = (np.mean(Z_2x, axis=1) - (w_best.transpose() @ np.mean(X_2x, axis=1).reshape(2*N, 1))).reshape(d, 1)
...
def prediction_mj(state_dict, reg_result, testing_data, A, gamma, C, s, zeta, d, platt_err):
    prediction_dict = {'testing_error': None,
                       'z_actuals': None,
                       'z_predictions': None,
                       'states': None}
    ...
    T_bar = len(testing_data)
    T = state_dict.get('input_data').shape[1]
    N = state_dict.get('last_state').shape[0]
    
    x_t = state_dict.get('last_state')
    x_t_1 = state_dict.get('all_states')[:, -2].reshape(N, 1)
    z_predict = state_dict.get('input_data')[:, -1].reshape(d, 1)
    ...
    for t_bar in range(0, T_bar):
        x_t_1 = x_t
        x_t = state(x_t_1, z_predict, A, gamma, C, s, zeta, d)
        z_predict = observation_mj(x_t, x_t_1, reg_result)
        ...
        if t_bar == 0:
            ...
            z_predictions = np.concatenate((z_predictions, z_predict), axis=1)
            z_actuals = np.concatenate((z_actuals, z_actual), axis=1)
            states = np.concatenate((states, x_t), axis=1)
...
def training_error_mj(state_dict, reg_result, training_data, T_trans):
    training_error_dict = {'training_error': None,
                           'z_actuals': None,
                           'z_predictions': None}
    ...
    for t in range(0, T-T_trans-1):
        x_t = X_2x[0:N, t].reshape(N, 1)
        x_t_1 = X_2x[N:2*N, t].reshape(N, 1)
        z_predict = observation_mj(x_t, x_t_1, reg_result).reshape(d, 1)
        z_actual = Z_2x[:, t].reshape(d, 1)  
    ...
    training_error = training_error/(T-T_trans-1)
