...
%time state_dict_850 = esn_help.listening(training_data, x_0, A, gamma, C, s, zeta, d, N)
%time reg_result_850 = esn_help.regression_covariance(ld, state_dict_850, step_trans)
%time predicting_850 = esn_help.prediction(state_dict_850, reg_result_850, testing_data, A, gamma, C, s, zeta, d, N)

training_error_850 = esn_help.training_error(state_dict_850, reg_result_850, step_trans)
print('Training error: ', training_error_850)
print('Testing error: ', predicting_850['testing_error'])

# %% Lyapunov spectrum using nolds

data = predicting['z_predictions'].flatten(order="C")
data_850 = predicting_850['z_predictions'].flatten(order="C")
%time esn_spectrum = nolds.lyap_e(data, emb_dim=3999, matrix_dim=2000)
%time esn_850_spectrum = nolds.lyap_e(data_850, emb_dim=1699, matrix_dim=850)
%time data_spectrum = nolds.lyap_e(training_data, emb_dim=1699, matrix_dim=850)

# %% plot spectrum

esn_spectrum_sorted = sorted(esn_spectrum, reverse=True)
esn_spectrum_850_sorted = sorted(esn_850_spectrum, reverse=True)
...
esn_plot_range = esn_spectrum_850_sorted
esn_index = np.arange(0, len(esn_plot_range))
plt.scatter(esn_index, esn_plot_range, s=2, marker='x', label='ESN 850')
